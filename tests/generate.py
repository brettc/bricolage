...
_numpy_dumps = dict([(nm, (data_dir / nm).with_suffix(".npy")) for nm in ["noisy"]])
...
def perturb(overwrite):
    ...
    with SnapshotLineage(dbpath, params=p) as lin:
        if not lin.targets:
            lin.targets.append(
                NoisyTarget(
                    lin.world, perturb_target, perturb_count=3, perturb_prop=0.2
                )
            )
...
def three(overwrite):
    ...
    with SnapshotLineage(dbpath, params=p) as lin:
        if not lin.targets:
            lin.targets.append(
                NoisyTarget(lin.world, three_target, perturb_count=1, perturb_prop=0.1)
            )
...
def make_noisy():
    ...
    ntarget = NoisyTarget(world, bowtie_target, perturb_count=3, perturb_prop=0.2)
