...
class Replicate(object):
    ...
    def __repr__(self):
        return "<Replicate: {}-{} Seed-{}>".format(
            self.treatment.name, self.seq, self.seed
        )
    ...
    def get_lineage(self, readonly=False):
        ...
        if not p.exists():
            if readonly:
                raise ExperimentError(
                    "Replicate doesn't exist, {}".format(self.path.name)
                )
        ...
        if db_path.exists():
            lin = self.treatment.experiment.lineage_class(db_path, readonly=readonly)
        ...
        if self.fresh:
            ...
            lin = self.treatment.experiment.lineage_class(
                db_path, self.params, init_pop_fun=init_pop_fun
            )
    ...
    def draw_winners(
        self, lin, maxw=1, graph_type=GraphType.GENE, with_dot=False, knockouts=True
    ):
...
class Treatment(object):
    ...
    def __repr__(self):
        return "<Treatment: {} R-{} Seed-{}>".format(self.name, self.count, self.seed)
    ...
    def run_variable(
        self, replicate, lineage, targfun, streak_len, max_gens, log_every=1000
    ):
...
class Experiment(object):
    def __init__(
        self, path, treatments, name=None, seed=1, analysis_path=None, full=True
    ):
    ...
    def visit_lineages(self, visitor, only_treatment=None, only_replicate=None):
                ...
                with rep.get_lineage(readonly=True) as lin:
                    ...
                    if only is not None:
                        ...
                        gen_num = (
                            gen_index
                            if pos_indexing
                            else lin.generation + gen_index + 1
                        )
                        last_gen = False
